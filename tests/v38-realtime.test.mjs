...
try {
  ...
  ok(captured[0]?.session?.model === 'gpt-realtime-2.1', 'upstream session receives the current default model');
  ...
  ok(captured[0]?.session?.audio?.output?.voice === 'marin', 'recommended current voice remains the safe default');
