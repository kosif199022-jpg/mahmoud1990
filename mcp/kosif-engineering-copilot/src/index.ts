...
function createServer() {
  const server = new McpServer({
    name: "KOSIF Engineering Copilot",
    version: "0.2.0",
  });
  ...
  server.registerTool(
    ...
    async ({ limit }) => {
      const prs = await github(
        `/pulls?state=open&sort=updated&direction=desc&per_page=${limit}`,
      );
      const data = {
  ...
  server.registerTool(
    ...
    async ({ prNumber }) => {
      ...
      const failing = checkRuns.filter((c: any) =>
  ...
  server.registerTool(
    ...
    async ({ ref }) => {
      ...
      requireMarker(/^\s*<!doctype html>/i.test(html), "error", "DOCTYPE", "HTML5 doctype is missing.");
      requireMarker(/<html[^>]*\blang=["']ar["']/i.test(html), "error", "LANG_AR", "Root html should declare Arabic language.");
      requireMarker(/<html[^>]*\bdir=["']rtl["']/i.test(html), "error", "RTL", "Root html should declare RTL direction.");
      requireMarker(/viewport-fit=cover/i.test(html), "error", "VIEWPORT_FIT", "iPhone viewport-fit=cover marker is missing.");
      requireMarker(/:focus-visible\b/i.test(html), "warning", "FOCUS", "No :focus-visible rule detected.");
      requireMarker(/prefers-reduced-motion\s*:\s*reduce/i.test(html), "warning", "REDUCED_MOTION", "Reduced-motion handling is missing.");
      requireMarker(/(margin-inline|padding-inline|inset-inline)/i.test(html), "warning", "LOGICAL_CSS", "No logical RTL CSS properties detected.");
      requireMarker(/overflow-x\s*:\s*auto/i.test(html), "warning", "HORIZONTAL_SCROLL", "No horizontal overflow protection detected.");
      requireMarker(/--pine\s*:/i.test(html) && /--seal\s*:/i.test(html), "warning", "DESIGN_TOKENS", "Core KOSIF design tokens were not detected.");
      requireMarker(/@media\b/i.test(html), "warning", "RESPONSIVE", "No responsive media query detected.");

      const modalMentions = (html.match(/\bmodal\b/gi) || []).length;
      if (modalMentions > 0) {
        requireMarker(
  ...
  server.registerTool(
    ...
    async ({ ref }) => {
      ...
      const failingWorkflows = workflowRuns.filter((r: any) => badConclusions.has(r.conclusion));
      const pendingWorkflows = workflowRuns.filter((r: any) => r.status !== "completed");
      const failingChecks = checkRuns.filter((r: any) => badConclusions.has(r.conclusion));
      const pendingChecks = checkRuns.filter((r: any) => r.status !== "completed");
      const evidenceVisible = workflowRuns.length > 0 || checkRuns.length > 0;
      const productionHealthy = health?.productName === "Kosif";

      const blockers: string[] = [];
      if (!evidenceVisible) blockers.push("No CI/check evidence is visible for this ref.");
      ...
      const data = {
        ref,
        sha,
        readiness: ready ? "READY_FROM_VISIBLE_EVIDENCE" : "BLOCKED_OR_INCOMPLETE",
        blockers,
        ci: {
