/* Kosif — Kitab Caffe theme authority.
   Keeps the user-approved visual layer last after late UI injections and
   honours the application's explicit data-theme state on iOS/desktop. */
(() => {
  'use strict';
  ...
  const explicitMode = () => {
    const v = String(root.dataset.theme || '').toLowerCase();
    return v === 'dark' ? 'dark' : (v === 'light' || v === 'sepia') ? 'light' : '';
  };
  const applyExplicitMode = () => {
    const mode = explicitMode();
    const vars = mode === 'dark' ? DARK : mode === 'light' ? LIGHT : null;
    for (const key of Object.keys(LIGHT)) {
      if (vars) root.style.setProperty(key, vars[key]); else root.style.removeProperty(key);
    }
    root.style.colorScheme = mode || '';
  };
  const isDark = () => explicitMode() === 'dark' || (!explicitMode() && dark && dark.matches);
  const paintChrome = () => {
    const colour = isDark() ? PAPER_DARK : PAPER;
  ...
  const sync = () => { applyExplicitMode(); paintChrome(); pin(); };
  sync();
