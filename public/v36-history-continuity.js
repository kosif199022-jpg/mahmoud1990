...
function makePalette(){if(q('#kosif-command-palette'))return q('#kosif-command-palette');const d=document.createElement('div');d.id='kosif-command-palette';d.className='kosif-overlay';d.innerHTML=`<div class="kosif-palette" role="dialog" aria-modal="true" aria-label="البحث والأوامر"><div class="kosif-palette-input"><span>⌕</span><input id="kosif-command-input" autocomplete="off" placeholder="اكتب قسمًا أو أمرًا…"><button type="button" class="kosif-x" data-kosif-close>✕</button></div><div id="kosif-command-results"></div><div class="kosif-palette-hint">Ctrl / ⌘ + K للفتح · Esc للإغلاق</div></div>`;document.body.appendChild(d);d.addEventListener('click',e=>{if(e.target===d||e.target.closest('[data-kosif-close]'))closePalette()});q('#kosif-command-input').addEventListener('input',renderPalette);q('#kosif-command-input').addEventListener('keydown',e=>{if(e.key==='Enter')q('#kosif-command-results button')?.click()});return d}
...
function bind(){
 ...
 document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();openPalette()}if(e.key==='Escape'){closePalette();closeMore();closeAppearance();tourStop=true;try{speechSynthesis.cancel()}catch(_){}}});
