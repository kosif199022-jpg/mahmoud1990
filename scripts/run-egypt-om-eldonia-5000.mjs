...
const creditNature = accounts.filter(a => ['liability','equity','revenue','suspense'].includes(a.type));
const suspenseAccount = accounts.find(a => a.type === 'suspense').code;

// One deterministic opening entry touches every account and is balanced exactly.
const openingLines = [];
...
if (openingDr > openingCr) {
  const delta = openingDr - openingCr;
  const line = openingLines.find(x => x.account === creditNature[0].code);
  line.cr = major(BigInt(String(line.cr).replace(/,/g,'')) * 100n + delta); // replaced below safely
} else if (openingCr > openingDr) {
  const delta = openingCr - openingDr;
  const line = openingLines.find(x => x.account === debitNature[0].code);
  line.dr = major(BigInt(String(line.dr).replace(/,/g,'')) * 100n + delta); // replaced below safely
}
// Recompute opening totals from strings to avoid trusting construction after balancing adjustment.
...
if (od !== oc) {
  const delta = od > oc ? od - oc : oc - od;
  const target = od > oc ? openingLines.find(x => x.account === creditNature[0].code) : openingLines.find(x => x.account === debitNature[0].code);
  if (od > oc) target.cr = major(parseMajorToMinor(target.cr) + delta);
  else target.dr = major(parseMajorToMinor(target.dr) + delta);
}
const openingEntry = {id:'OPEN-2026', date:'2026-01-01', memo:'أرصدة افتتاحية اصطناعية — 5,000 حساب', user:'system-demo', source:'synthetic', lines:openingLines};
...
// 1,500 subsequent balanced entries exercise normal and high-risk journal patterns.
const journals = [];
for (let i = 1; i <= 1500; i++) {
  const anomaly = i % 75 === 0;
  const debit = anomaly ? accountMap.get(suspenseAccount) : pick(debitNature);
  const credit = pick(creditNature.filter(a => a.code !== debit.code));
...
const journalRiskResults = journals.map(j => ({id:j.id, ...journalRiskFlags(j, {suspenseAccount, amountThreshold:major(materiality.performance.minor), exp:2})}));
...
const sample = deterministicSample(journals.map((j, i) => ({id:j.id, amount:parseMajorToMinor(j.lines[0].dr || '0').toString(), index:i})), {size:150, seed:String(SEED), method:'systematic'});
assert(sample.picked.length === 150, 'SYSTEMATIC_SAMPLE_SIZE_MISMATCH', sample.picked.length);
const mus = deterministicSample(journals.map((j, i) => ({id:j.id, amount:Number(parseMajorToMinor(j.lines[0].dr || '0') / 100n), index:i})), {size:120, seed:String(SEED + 1), method:'mus', amountKey:'amount'});
...
assert(misstatementAgg && misstatementAgg.total, 'MISSTATEMENT_AGGREGATION_FAILED');

// 40 valid proposed adjustments + 1 intentionally invalid adjustment to prove fail-closed behavior.
const adjustments = [];
for (let i = 1; i <= 40; i++) {
...
assert(invariantResult.allOk, 'LEDGER_INVARIANTS_FAILED', invariantResult);
const frameworks = frameworkApplicability({periodStart:COMPANY.periodStart, periodEnd:COMPANY.periodEnd});
...
const procedures = [
  ...
  ['ledger_integrity','PASS','hash-chain/invariants سليمة'],
  ['egypt_tax_boundary','PASS','لم يتم تطبيق ZATCA VAT/زكاة السعودية على شركة مصرية؛ أي ضريبة محلية مصرية تحتاج محركًا/مصدرًا مصريًا صريحًا'],
  ['ai_council_network','SECURITY-SKIP','لا تُستدعى مفاتيح مزودي AI من اختبار آلي؛ تغطي بوابات KOSIF العامة حوكمة المجلس والاعتماد البشري'],
