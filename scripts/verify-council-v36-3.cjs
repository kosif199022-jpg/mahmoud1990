...
(async()=>{
  ...
  const healthRes=await page.request.get(base+'/__health?cb='+Date.now());
  if(!healthRes.ok())fail('Health endpoint unavailable',healthRes.status());
  const health=await healthRes.json();
  const declaredAudit=String(health?.modules?.audit||'').trim();
  const auditPath=declaredAudit.startsWith('/') ? declaredAudit : (/^v3[7-9]\./.test(String(health.version||''))?'/audit/':'/');
  const nav=await page.goto(base+auditPath+'?council-v2='+Date.now(),{waitUntil:'domcontentloaded',timeout:30000});
  if(!nav||!nav.ok())fail('Audit workspace navigation failed',nav?.status?.());
  await page.waitForSelector('body.kosif-ready',{timeout:20000});
  ...
  await page.waitForFunction(()=>{const x=document.querySelector('#view-council.show>.card');if(!x)return false;const r=x.getBoundingClientRect(),s=getComputedStyle(x);return r.width>0&&r.height>0&&Number(s.opacity)>.95&&s.visibility!=='hidden'},{timeout:5000});
  ...
  await page.waitForTimeout(180);const intl=await page.locator('#cv2-context').innerText();console.log('COUNCIL_INTL',intl);if(!/دولي/.test(intl)||/السعودية/.test(intl))fail('Council did not follow international jurisdiction',intl);
  if(errors.length)fail('Council page errors',errors);
  await page.screenshot({path:'kosif-v36-3-council-v2.png',fullPage:true});
  await browser.close();console.log('KOSIF_V36_3_COUNCIL_V2_OK',health.version,auditPath);
