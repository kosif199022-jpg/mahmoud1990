...
ok('Phase B stylesheet is dynamically loaded',/v36-mobile-phase-b\.css\?v=36\.4-phase-b-3-fixed-context/.test(js)&&/loadPhaseBStyles/.test(js));
...
ok('iOS app-shell lock remains exact',/lockY=Math\.max\(0,Number\.isFinite\(preferredY\)\?preferredY:\(window\.scrollY\|\|document\.documentElement\.scrollTop\|\|0\)\)/.test(js)&&/p\.style\.top=`-\$\{lockY\}px`/.test(js)&&/window\.scrollTo\(0,y\)/.test(js));
...
ok('hoisted drawer has explicit closed and open viewport states',/body>\.drawer:not\(\.open\)/.test(css)&&/body>\.drawer\.open\{transform:none!important/.test(css));
