...
ok(proxy.includes('function requestedLibraryBook(url)')&&proxy.includes("return LIBRARY_BOOKS.has(book) ? book : ''"),'one allowlist helper owns reader book identity');
ok(proxy.includes('data-kosif-book-bootstrap="${book}"')&&proxy.includes('const bookMarker = requestedBook ? `data-kosif-book-bootstrap="${requestedBook}"`'),'each reader bootstrap is marked with its exact requested book id');
ok(proxy.includes('if (bookBoot && !text.includes(bookMarker)) injections.push(bookBoot)')&&!proxy.includes("bookBoot && !text.includes(\"localStorage.setItem('mk_lib_book'\")"),'a bootstrap for another book cannot suppress the requested book deep link');
...
ok(proxy.includes('const requestedBook = requestedLibraryBook(requestUrl)')&&proxy.includes("u.searchParams.set('book', requestedBook)"),'fallback reader redirects preserve explicit allow-listed book selection');
