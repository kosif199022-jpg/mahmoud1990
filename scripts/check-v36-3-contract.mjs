...
ok('mobile nav primary routes',JSON.stringify(nav)===JSON.stringify(['overview','tb','rounds','outputs']));
ok('mobile nav labels exact',/الرئيسية<\/button>[\s\S]*الميزان<\/button>[\s\S]*الجولات<\/button>[\s\S]*المخرجات<\/button>[\s\S]*المزيد<\/button>/.test(html));
