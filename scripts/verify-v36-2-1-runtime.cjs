...
const expectedVersion='v'+String(pkg.version||'').split('.').slice(0,2).join('.');
...
const expectedBuild=String(releaseManifest.buildId||'');
(async()=>{
  ...
  ok(h.version===expectedVersion,'wrong health version: expected '+expectedVersion);
  ok(h.buildId===expectedBuild,'wrong build id: expected '+expectedBuild);
  ...
  await page.goto(BASE+'/?v='+encodeURIComponent(expectedVersion),{waitUntil:'domcontentloaded'});await page.waitForTimeout(1800);
  ...
  await page.goto(BASE+'/standards/?v='+encodeURIComponent(expectedVersion),{waitUntil:'domcontentloaded'});await page.waitForTimeout(900);
  ...
  await page.screenshot({path:'kosif-v36-3-mobile.png',fullPage:true});await browser.close();console.log('KOSIF_DEEP_VERIFY_OK',expectedVersion,expectedBuild);
