...
(async()=>{
  ...
  ok(h.version==='v36.3','wrong v36.3 health version');
  ok(h.buildId==='2026.08.17-v36.3-master-requirements','wrong v36.3 build id');
  ok(/Z\.ai/.test(h.release||''),'Z.ai release marker missing');
  ...
  await page.goto(BASE+'/?v=36.3',{waitUntil:'domcontentloaded'});await page.waitForTimeout(1800);
  ...
  await page.screenshot({path:'kosif-v36-3-mobile.png',fullPage:true});await browser.close();console.log('KOSIF_V36_3_DEEP_VERIFY_OK');
