...
(async()=>{
  ...
  ok(h.version==='v36.2.1','wrong v36.2.1 health version');
  ok(h.release==='Library Integrity & Motion Hardening','wrong release marker');
  ...
  await page.goto(BASE+'/?v=36.2.1',{waitUntil:'domcontentloaded'});await page.waitForTimeout(1800);
  ...
  await page.locator('#kosif-ai-status').click();await page.waitForTimeout(120);const key=page.locator('#kai-key');ok(await key.getAttribute('readonly')!==null,'AI key must be locked');ok((await key.inputValue())==='','locked AI key must be empty');ok(await page.locator('#kai-test').isDisabled(),'AI test enabled while locked');
  ...
  await page.goto(BASE+'/standards/?v=36.2.1',{waitUntil:'domcontentloaded'});await page.waitForTimeout(900);
  ...
  await page.screenshot({path:'kosif-v36-2-1-mobile.png',fullPage:true});await browser.close();console.log('KOSIF_V36_2_1_DEEP_VERIFY_OK');
