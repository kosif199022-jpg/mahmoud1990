...
if (!build.includes(delayed) || !build.includes(direct)) throw new Error('Direct-book build transform is missing');
if (!reader.includes(delayed) && !reader.includes(direct)) throw new Error('Wealth reader requested-book bootstrap contract is missing');
const simulatedBuild = reader.includes(delayed) ? reader.replace(delayed, direct) : reader;
if (simulatedBuild.includes(delayed) || !simulatedBuild.includes(direct)) throw new Error('Build transform does not remove the 700ms requested-book delay');
