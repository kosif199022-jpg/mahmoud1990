...
async function wealthBookData(req,env,u){
  ...
  if(p==='/wealth/books/library.json'){
    const r=await staticAsset(req,env,'/wealth/books/library.json');
    return r||json({error:'LIBRARY_INDEX_NOT_BUILT'},503);
  }
  const idx=p.match(/^\/wealth\/books\/(std2018|std2025|dipifr)\.json$/);
  if(idx){
    const r=await staticAsset(req,env,`/wealth/books/${idx[1]}.json`);
    return r||json({error:'BOOK_INDEX_NOT_BUILT',book:idx[1]},503);
  }
  const chapter=p.match(/^\/wealth\/books\/(std2018|std2025|dipifr)\/(\d+)\.json$/);
  if(chapter){
    const id=chapter[1],n=Number(chapter[2]),cfg=READER_BOOKS[id];
    if(!Number.isInteger(n)||n<1||n>500)return json({error:'INVALID_CHAPTER'},400);
    return redirect(req,`/standards/data/${cfg.source}/${n}.json`,307);
  }
...
export default{async fetch(req,env,ctx){const u=new URL(req.url),p=u.pathname;if(p==='/__suite')return json({ok:true,...SUITE});if(p==='/__version')return json({...SUITE,legacyRelease:'v36.4-compatible'});if(p==='/__health')return json({ok:true,name:'Kosif',...SUITE,security:{companyData:'owner-only',customSourceFetch:'allowlist-only',ai:'owner+verified-provider'}});if(req.method==='GET'&&(p==='/'||p==='/hub'||p==='/hub/'))return hub(req,env);if(p==='/audit')return redirect(req,'/audit/');if(req.method==='GET'&&(p==='/audit/'||p==='/audit/index.html'))return auditShell(req,env,ctx);if(p==='/libraries')return redirect(req,'/libraries/');if(req.method==='GET'&&(p==='/libraries/'||p==='/libraries/index.html'))return libraryShell(req,env);if(p.startsWith('/libraries/')){const r=await staticAsset(req,env,p);if(r)return decorate(r,'libraries')}if(p==='/wealth')return redirect(req,'/wealth/reader.html');if(req.method==='GET'&&p.startsWith('/wealth/books/'))return decorate(await wealthBookData(req,env,u),'wealth-library');if(p.startsWith('/wealth/')){const r=await proxyWealth(req,env);if(r)return decorate(r,'wealth')}if(req.method==='GET'&&p.startsWith('/standards/data/')){const r=await staticAsset(req,env,p);if(r)return decorate(r,'standards-data')}if(p==='/sales')return redirect(req,'/sales/');if(req.method==='GET'&&(p==='/sales/'||p==='/sales/index.html'))return salesShell(req,env);if(p==='/modules'||p==='/modules/')return redirect(req,'/sales/modules/');if(p.startsWith('/modules/')){const target='/sales'+p;const r=await staticAsset(req,env,target.endsWith('/')?target+'index.html':target);if(r)return decorate(r,'sales');return new Response('Not found',{status:404})}const blocked=await privacyGate(req,env,u);if(blocked)return blocked;const r=await securityEdge.fetch(req,env,ctx);return p.startsWith('/api/')?sameOriginApi(r,req):decorate(r,p.startsWith('/sales/')?'sales':'kosif')}};
