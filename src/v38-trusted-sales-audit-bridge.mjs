 ...
 * كل النتائج مشتقة من البيانات الداخلة فقط.
 */
...
export function analyzeSalesAuditBridge(rows = []) {
  ...
  for (const row of sales) {
    ...
    seen.add(key);
    if (Number(row.cost || 0) > Number(row.revenue || 0)) {
      exceptions.push({ type: 'cost_exceeds_revenue', product: row.product || null });
    }
  }
  ...
  return {
    ...
    governance: {
      ...
      note: 'تحليل تشغيلي فقط؛ الترحيل والمراجعة النهائية عبر الضوابط الحتمية والمراجع البشري.'
    }
  };
}
