...
async function a(req,env){if(!env?.ASSETS)return null;try{let assetReq=req;const u=new URL(req.url);if(u.pathname==='/'){const indexUrl=new URL('/index.html',u);indexUrl.search=u.search;assetReq=new Request(indexUrl,req)}const r=await env.ASSETS.fetch(assetReq);return r.status===404?null:r}catch{return null}}
