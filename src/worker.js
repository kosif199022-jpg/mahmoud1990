...
function tag(r){const h=new Headers(r.headers);h.set('x-kosif-release','native-v36-2-attachments-upgrade');h.set('x-content-type-options','nosniff');h.set('referrer-policy','strict-origin-when-cross-origin');return new Response(r.body,{status:r.status,statusText:r.statusText,headers:h})}
...
export default{async fetch(req,env,ctx){
  ...
  if(u.pathname==='/__health')return Response.json({ok:true,name:'Kosif Native',version:'v36.2',release:'Attachment Review & Operational Intelligence',architecture:'worker-first-static-assets',aiGate:'owner-password+verified-key'});
